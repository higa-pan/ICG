...
if __name__ == "__main__":
    ...
    data_path = abs_root / "random_result_data"
    ...
    for rand_seed in rand_seeds:  # 1回の実験をSIMULATION_LOOP_TIMES(用意したseedの数だけ)回行う
        ...
        # データをとる最後の1万ステップ
        simulate.reset_sequence_data()              # 1.エージェントの協力先・グループへの協力数のデータをリセット
        simulate.play_game(10000)       # 2.ICゲームを行う(行動決定、Qtableの更新を含む)
        simulate.save_sequence_data(const.SIMULATION_TIMES // 10000 - 1, str(rand_seed_path))  # 3.エージェントの協力先・グループへの協力数のデータを保存
