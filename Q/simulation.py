...
class Simulation:
    ...
    def append_q_table(self):
        self.agent_q_table.append(np.ravel(self.agent_list[0].Q_table).copy())

    def update_q_table(self):
        for i in range(self.num_agent):
            agent_members = list(self.agent_list[i].group_member_map.values())
            self.agent_list[i].view_other_action(self.agent_list[agent_members[0]], self.agent_list[agent_members[1]])
            self.agent_list[i].update_q_table()

    def play_game_add_q_table(self, steps):
        for step in range(steps):
            self.append_q_table()
            self.determine_act()
            ic_game(self.agent_list, self.group_member_map, self.group_payoff, self.group_cooperate_times, self.agent_cooperate_times)
            self.update_q_table()
    ...
    def save_sequence_data(self, Term, Save_path):
        make_sequence_data(self.group_cooperate_times, self.agent_cooperate_times, self.agent_q_table, Term, Save_path)
...
def make_sequence_data(group_coop_list, agent_coop_list, agent_q_table_list, Term, Save_path):
    ...
    data = pd.DataFrame(agent_q_table_list)
